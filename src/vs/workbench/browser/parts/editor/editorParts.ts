 ...
 *--------------------------------------------------------------------------------------------*/
import {
	EditorGroupLayout, GroupDirection, GroupOrientation, GroupsArrangement,
	GroupsOrder, IAuxiliaryEditorPart, IAuxiliaryWindowOpenOptions, IEditorDropTargetDelegate,
	IEditorGroupsService, IEditorSideGroup, IFindGroupScope, IMergeGroupOptions
...
export class EditorParts extends Disposable implements IEditorGroupsService, IEditorPartsView {
	...
	createAuxiliaryEditorPart(options?: IAuxiliaryWindowOpenOptions): IAuxiliaryEditorPart {
		...
		const auxiliaryWindow = this.auxiliaryWindowService.open(options);

		const partContainer = document.createElement('div');
		partContainer.classList.add('part', 'editor');
		partContainer.setAttribute('role', 'main');
		// partContainer.innerText = 'Auxiliary Editor Part';
		...
		const noTitle = (!this.configurationService.getValue('window.auxWindow.hasTitle') || (getTitleBarStyle(this.configurationService) !== 'custom'));
		const titleBarHeight = noTitle ? 0 : 30;

		auxiliaryWindow.onDidResize(dimension => {
			editorPart.layout(dimension.width, dimension.height - titleBarHeight - 1, 0, titleBarHeight);
		});

		auxiliaryWindow.onDidClose(() => {
			this.auxiliaryWindowService.close(auxiliaryWindow);
			// 释放对editorPart相关的事件的监听
			disposables.dispose();
		});
...
registerSingleton(IEditorGroupsService, EditorParts, false);
